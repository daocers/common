...
<div class="container">
    ...
    <div class="row table-responsive">
        <table class="table table-bordered">
            ...
            <tbody>
            <c:forEach items="${pi.data}" var="systemparam" varStatus="line">
                <tr>
                    <td><input type="checkbox" objId="${systemparam.id}"></td>
					                        <td>${systemparam.code}</td>
					                        <td>${systemparam.description}</td>
					                        <td>${systemparam.name}</td>
					                        <td>${systemparam.value}</td>
					                        <td>${systemparam.valueType}</td>
					                    <td>
                        <a href="edit.do?id=${systemparam.id}&type=detail" class="opr">详情</a>
                        <a href="edit.do?id=${systemparam.id}" class="opr">修改</a>
                        <a href="javascript:del(${systemparam.id})" class="opr">删除</a>
                    </td>
                </tr>
            </c:forEach>
