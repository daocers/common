...
<div class="container">
    ...
    <div class="row table-responsive">
        <table class="table table-bordered">
            ...
            <tbody>
            <c:forEach items="${pi.data}" var="propertyitem" varStatus="line">
                <tr>
                    <td><input type="checkbox" objId="${propertyitem.id}"></td>
					                        <td>${propertyitem.code}</td>
					                        <td>${propertyitem.idx}</td>
					                        <td>${propertyitem.name}</td>
					                        <td>${propertyitem.tesPropertyId}</td>
					                        <td>${propertyitem.value}</td>
					                        <td>${propertyitem.itemsIdx}</td>
					                    <td>
                        <a href="edit.do?id=${propertyitem.id}&type=detail" class="opr">详情</a>
                        <a href="edit.do?id=${propertyitem.id}" class="opr">修改</a>
                        <a href="javascript:del(${propertyitem.id})" class="opr">删除</a>
                    </td>
                </tr>
            </c:forEach>
