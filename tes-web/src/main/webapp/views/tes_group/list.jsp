...
<div class="container">
    ...
    <div class="row table-responsive">
        <table class="table table-bordered">
            ...
            <tbody>
            <c:forEach items="${pi.data}" var="group" varStatus="line">
                <tr>
                    <td><input type="checkbox" objId="${group.id}"></td>
					                        <td>${group.name}</td>
					                        <td>${group.description}</td>
					                        <td>${group.branchId}</td>
					                        <td>${group.departmentId}</td>
					                        <td>${group.stationId}</td>
					                        <td>${group.createTime}</td>
					                        <td>${group.createUserId}</td>
					                        <td>${group.status}</td>
					                    <td>
                        <a href="edit.do?id=${group.id}&type=detail" class="opr">详情</a>
                        <a href="edit.do?id=${group.id}" class="opr">修改</a>
                        <a href="javascript:del(${group.id})" class="opr">删除</a>
                    </td>
                </tr>
            </c:forEach>
